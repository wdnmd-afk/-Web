...
### 排查记录：「媒体资源连接失败 … EOF」与清晰度无关

用户反馈此提示"经常"出现并怀疑是清晰度导致。实测否定该假设，同时否掉了一个一度打算实施的改法。

**采样一（档位相关性）**：同一部剧、每次全新会话并即时释放、档位顺序打乱，15 次取流。
...
480/720/1080 每档均既有成功也有失败，失败率不随档位变化。7 次失败全为同一主机 `v5-se-sjy-daily-reading-video-a.qznovelvod.com` 的 `EOF`。附注：本剧 720 与 1080 成功时字节数完全一致（35663153），因为**这一部剧**的 1080p 档真实像素只有 720×1280；此结论仅限该剧，不可推广（见下一节）。

**采样二（是否全站）**：6 部剧各取流一次，5 部正常，仅 1 部失败。失败按资源聚集，非全站故障。

**采样三（时间性）**：先前 7/15 失败的那部剧，换时段 8 次全部首次成功。同一资源成功率随时间显著变化。

**采样四（重取是否有效）**：对稳定失败的剧（`7682341088467618841`）做 6 轮「失败→重新解析播放地址→再试」，结果首次成功 0 / 重取挽回 0 / 全败 6。

**结论**：`EOF` 发生在响应头之前，属传输层失败——服务器接受连接后针对该具体请求断开，与转码、解码、清晰度无关。成因在上游：按资源聚集且带时间性。已排除清晰度、网络路径（直连该主机 TCP 0.03s、TLS 0.07s，五次全通）、URL 过期（采样一每次均为全新解析地址）、全站故障。

**据此否掉的改法**：原打算在连接级失败后重新解析播放地址再试。采样四证明挽回率为零，只会在失败时多打一次红果 API，不实施。用户点「重试播放」有时有效，靠的是时间间隔而非新 URL（已证伪）。

**待定的小优化**：`download_hls.go:261-271` 对同一 URL 重试 3 次（间隔 1s、2s）。上述失败模式下三次必然全败，等待纯属浪费，报错前多卡约 3 秒。可将连接级 `EOF` 判为不可重试立即返回，但该路径下载复用，改动会影响下载重试行为，需确认后再动。

**排查中纠正的三处自身误判**：

1. 探针连开 15 个会话未释放，撞上 `ui_playback.go:18` 的 `playbackSessionLimit = 4`，12 条 `429` 是探针缺陷而非上游限流，不可作数据。
2. `newHLSProxy` 在 `playback_stream.go:150` 无条件创建，非仅用于 `.m3u8`，故纯 MP4 地址亦经此代理，会报出 HLS 族错误。
3. 错误信息中主机名与失败 URL 不一致属正常：前者取自我方发起请求的主机，后者为 Go `url.Error` 携带的最终 URL。

### 排查记录：清晰度档位是否真实有效、是否存在更高档

用户追问「清晰度是否真实有效」「有没有更高清的」。逐档拉真实流后用自带 FFmpeg 读实际输出像素。

**档位有效性（剧 A `7684913948310637592`，同一集逐档实测）**：

| 档位 | 实际输出像素 | 码率 |
| --- | --- | --- |
| 360 | 360×640 | 711 kb/s |
| 480 | 480×852 | 997 kb/s |
| 540 | 540×960 | 1137 kb/s |
| 720 | 720×1280 | 1618 kb/s |
| 1080 | 720×1280 | 1618 kb/s |

360→720 逐档单调上升，档位切换真实改变输出。1080 与 720 完全一致（字节数同为 35663153），因为该剧 1080p 档真实像素只有 720×1280，而 `playback_stream.go:88` 的 scale 用 `min(iw,…)` 只缩不放。

**是否存在更高档：取决于剧集**。用临时测试拉原始 `video_list`（该数据无 HTTP 接口暴露，测试已删除）：

| 剧 | 1080p 档真实像素 |
| --- | --- |
| A `7684913948310637592` | 720×1280（标称虚高） |
| B `7684208891772537881` | 1080×1920（真实） |

剧 B 端到端实测确认：1080 档实际输出 **1080×1920 / 1901 kb/s**（41126521 B）。故 1080 选项对部分剧真实有效，非空档。

**顺带否掉一个猜测**：两部剧全部候选均为 bytevc1（H.265），无任何 bytevc2 被跳过。因此「更高分辨率藏在被 `provider_hongguo_native_media.go:76` 跳过的档位里」不成立，该跳过逻辑未屏蔽更清晰的档位。

**未取得的对照**：剧 B 的 720 档四次取流全部撞上上一节的 `EOF`，故无同剧 720 对照行。

**结论**：默认 720（`playbackMaxHeight`）对多数剧即上限画质；有真实 1080 源的剧手动选 1080 可多得一档，选后画面无变化即说明该剧 1080 为标称。
